...
kAddF (XFit v vv _) (HMeas h hh _) = kAddF' v (inv vv) h (inv hh) v (J.hv2q h v) 1e6 0

kAddF' :: Vec3 -> Cov3 -> Vec5 -> Cov5 -> Vec3 -> Vec3 -> Number -> Int -> XFit
kAddF' v0 uu0 h gg x_e q_e 𝜒2_0 iter = x_k where
    Jacs {aajacs=aa, bbjacs=bb, h0jacs=h0} = J.expand x_e q_e
    aaT   = tr aa
    bbT = tr bb
    x_k   = case invMaybe (bb .*. gg) of
              Nothing  -> XFit v0 (inv uu0) 1e6  `debug` "... can't invert in kAddF'"
              Just ww' -> x_k' where 
                ww    = ww'
                gb    = gg - gg .*. (bbT .*. ww)
                uu    = uu0 + aa .*. gb
...
kChi2 (XMeas v vv) (HMeas h hh w0) = Chi2 $ kChi2' x_km1 p_k x_e q_e 1e6 0 where
  x_km1 = XMeas v (inv vv)
  p_k   = HMeas h (inv hh) w0
  x_e   = v
  q_e   = J.hv2q h v
kChi2' :: XMeas -> HMeas -> Vec3 -> Vec3 -> Double -> Int -> Double
kChi2' (XMeas v0 uu0) (HMeas h gg w0) x_e q_e 𝜒2_0 iter = x_k where
  ...
  x_k   = case invMaybe (bb .*. gg) of
            Just ww' -> x_k' where 
              ...
              dm    = m - aa *. v
              q     = ww *. bbT *. gg *. dm
              𝜒2    = (v - v0) .*. uu0 -- or shoud it use uu?? + sw (dm - bb * q) gg
